...
const App: React.FC = () => {
  ...
  const behaviorStats = useMemo(() => {
    ...
    // FIX: Replaced forEach with for...of loop to avoid potential type narrowing issues in callbacks.
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm lg:col-span-2">
            ...
            <div className="mt-4 space-y-4">
                {Object.keys(stats).length > 0 ? Object.entries(stats).map(([subject, grades]) => {
                    // FIX: Explicitly type accumulator and value in reduce to fix type inference issue.
                    const totalGrades = (Object.values(grades) as number[]).reduce((a, b) => a + b, 0);
                    return (
                        <div key={subject}>
  ...
  const BehaviorTrendsChart = ({ data }: { data: { date: string, [BehaviorSentiment.POSITIVE]: number, [BehaviorSentiment.NEUTRAL]: number, [BehaviorSentiment.NEEDS_IMPROVEMENT]: number }[] }) => {
    // FIX: Using reduce to calculate maxCount to avoid spread operator issues.
  ...
  const renderMainDashboard = () => {
    // FIX: Explicitly type accumulator and value in reduce to fix type inference issue.
    const totalBehavior = Object.values(behaviorStats).reduce((a: number, b: number) => a + b, 0);
    return (
  ...
  const StudentBehaviorChart = ({ studentLogs, studentCount }: { studentLogs: LogEntry[], studentCount: number }) => {
    ...
    // FIX: Explicitly type accumulator and value in reduce to fix type inference issue.
    const totalStudent = Object.values(studentStats).reduce((a: number, b: number) => a + b, 0);
